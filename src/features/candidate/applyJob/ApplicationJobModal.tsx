...
export default function ApplicationModal(props: ApplicationJobModalProps) {
  ...
  useEffect(() => {
    if (userProfile.data) {
      reset({
        email: userProfile.data.data.profile.email || "",
        phone: userProfile.data.data.profile.phone || "",
        name: userProfile.data.data.profile.fullName || "",
        jobId: id,
      });
    }
  }, [userProfile.data, reset, id]);
