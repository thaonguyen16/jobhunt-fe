...
import SetPassword from "@features/candidate/setting/components/SetPassword";
...
export default function RecruiterInfo() {
  ...
  const handleSetPass = (pass: boolean) => {
    setPassword(pass)
  };
      ...
      >
        <Grid w={"100%"}>
          ...
          <Grid.Col span={{ base: 12, md: 4, sm: 12 }}>
            <UserInformation loading={handleSetLoading} password = {handleSetPass} reload={reload}></UserInformation>
            {password ? <ChangePassword loading={handleSetLoading} reload={reloadData} /> : <SetPassword loading={handleSetLoading} reload={reloadData}/>}
          </Grid.Col>
        </Grid>
      </Container>
    </>
