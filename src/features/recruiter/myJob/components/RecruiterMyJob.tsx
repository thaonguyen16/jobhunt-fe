...
import { DeleteJobModal, JobEditModal, StopJobModal } from "..";
...
export default function RecruiterMyJob() {
  ...
  const [stopJobId, setStopJobId] = useState<number | null>(null);
  ...
  const [editModalOpened, { open: openEditModal, close: closeEditModal }] =
    useDisclosure(false);
  const [stopModalOpened, { open: openStopModal, close: closeStopModal }] =
    useDisclosure(false);
  ...
  const handleOpenStopJobModal = (jobId: number) => {
    setStopJobId(jobId);
    openStopModal();
  };
        ...
        <Group grow wrap="nowrap" align="flex-start" mb="lg">
          ...
          >
            ...
            <Tabs.List mb="lg">
              ...
              <Tabs.Tab value="REJECTED">Bị từ chối</Tabs.Tab>
              <Tabs.Tab value="EXPIRED">Hết hạn</Tabs.Tab>
              <Tabs.Tab value="STOPPED">Ngưng tuyển</Tabs.Tab>
            </Tabs.List>
            ...
            <Tabs.Panel value={activeTab || "ACTIVE"}>
              <Stack gap="xs" align="stretch" justify="flex-start">
                ...
                {renderJobs &&
                  renderJobs.map((job) => (
                    <RecruiterJobCard
                      isActive={Number(job.id) === selectedJob}
                      key={job.id}
                      ...
                      setTotalJobApplications={setTotalJobApplications}
                      setSelectJobTitle={setSelectedJobTitle}
                      selectJob={setSelectedJob}
                      openDeleteModal={handleOpenDeleteModal}
                      openEditModal={handleOpenEditModal}
                      openStopModal={handleOpenStopJobModal}
