import { ActionIcon, Center, Text, Container, Flex, Grid, Group, Loader, Stack, Tooltip, Pagination } from "@mantine/core";
...
export default function FindEmployeeSection() {
  ...
  const handleFindEmployee = async(data: FilterCandidateCV) => {
    ...
    await fetchResumeFilter(currentPage, 5,data).then((data) => {
      if (data) {
  ...
  return (
    ...
    <>
      <Container maw={"2000px"} bg={"#f4f4f4"} pt={"20px"} mih={"500px"} pb={"100px"} ml={"0px"} className="filter-card" pos={"relative"}>
        {!hiddenFilter ? <Grid w={"100%"}>
          ...
          <Grid.Col span={{ base: 12, sm: 8.5, xs: 12 }}>
            <Stack gap={"sm"}>
              ...
              {!isloading ?<EmployeeCard resumData={resume} hiddenF={hiddenFilter} totalItem={totalItem}></EmployeeCard> : <Center w="100%" h="100%">
          <Loader color="green" type="dots" />
        </Center> }
              {resume && resume.length != 0 && !isloading &&  <Flex direction={"row"} align={"center"} justify={"center"} w={"100%"} mt={"1rem"} mb={"3rem"}>
